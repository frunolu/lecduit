...
$sql = "SELECT p.*, c.name$suffix as cat_name, s.name$suffix as sub_name FROM experiences p JOIN subcategories s ON p.subcategory_id = s.id JOIN categories c ON s.category_id = c.id WHERE p.id = :id AND p.is_active = 1";
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <div class="lg:sticky lg:top-32 bg-slate-50 p-10 rounded-[3rem] border border-slate-100 shadow-xl">
                ...
                <div class="space-y-4 mb-10 border-t py-8 border-slate-200">
                    <div class="flex items-center gap-4 text-slate-600 font-bold text-sm"><i class="fa fa-check text-green-500"></i> <?php echo $t['validity']; ?></div>
                    <div class="flex items-center gap-4 text-slate-600 font-bold text-sm"><i class="fa fa-envelope text-green-500"></i> <?php echo $t['delivery']; ?></div>
                </div>
                <div class="mb-10"><span class="text-[10px] font-black text-slate-400 uppercase italic block mb-1"><?php echo $t['price_for']; ?></span><div class="text-5xl font-black text-lec-dark italic"><?php echo formatPrice($p[$price_col]); ?></div></div>
                <form method="POST"><button type="submit" name="add_to_cart" class="w-full bg-lec-orange text-white py-6 rounded-3xl font-black text-xl shadow-xl hover:scale-[1.02] transition-all"><i class="fa fa-shopping-basket mr-2"></i> <?php echo $t['buy']; ?></button></form>
            </div>
        </div>
    </main>
</body>
